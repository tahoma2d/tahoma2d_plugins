 ...
 public:
  ...
  ParamPrototype const* param_prototype(int i) const override {
    static std::array<ParamPrototype const, PARAM_COUNT> params = {
        ...
        ParamPrototype{"x", PARAM_GROUP_DEFAULT, 0.5, 0, 1},
        ParamPrototype{"y", PARAM_GROUP_DEFAULT, 0.5, 0, 1},
    };
    return &params[i];
  }
...
int MyFx::kernel(Params const& params, Args const& args, cv::Mat& retimg) {
  ...
  // center
  float const cx = params.get<float>(PARAM_X, size.width);
  float const cy = params.get<float>(PARAM_Y, size.height);
